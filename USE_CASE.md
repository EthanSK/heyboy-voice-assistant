...
| MT-04 | Follow-up turn start clarity | Assistant gives short follow-up cue so user knows it is listening | Implemented | `FOLLOWUP_PROMPT` (default `"Go ahead."`) |
...
| MT-06 | Follow-up no speech (retries exhausted) | Assistant ends session with explicit “say wake phrase to continue” message | Implemented + Tested | `FOLLOWUP_TIMEOUT_PROMPT`, `test_followup_no_speech_ends_session` |
...
| STT-06 | STT backend misconfigured | Clear runtime error and safe exit | Implemented | `STT_BACKEND` validation in `main()` |
...
| BE-06 | CLI returns empty text | User gets explicit empty-response fallback | Implemented | `query_cli_backend()` |
| BE-07 | Backend returns blank/None | Assistant speaks configured fallback | Implemented | `EMPTY_BACKEND_REPLY` |
...
| OOS-02 | True streaming VAD endpointing | Current design is fixed listen windows for simplicity | Add streaming VAD + adaptive endpointing |
